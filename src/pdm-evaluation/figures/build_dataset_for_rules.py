...
dataset_characteristics_df = pd.DataFrame({
    "Dataset": ["CMAPSS", "FEMTO", "IMS", "EDP", "METRO", "Navarchos", "XJTU", "BHD", "AI4I", "AZURE"],
    # "type": ["s", "e", "e", "r", "r", "r", "e", "r", "s", "s"],
    "#records": [265256, 21493, 9464, 209236, 1516948, 854178, 9216, 6626869, 10000, 876100],
    "Min scenario length": [19, 172, 984, 52244, 1516948, 1482, 42, 11, 10000, 8761],
    "Avg scenario length": [187, 1264, 3154, 52309, 1516948, 32853, 614, 1529, 10000, 8761],
    "Std scenario length": [82, 772, 2806, 50, 0, 46824, 853, 776, 0, 0],
    "Min dimensions": [14, 44, 88, 79, 15, 6, 44, 9, 5, 4],
    "Max dimensions": [21, 44, 176, 79, 15, 6, 44, 32, 5, 4],
    "failures": [709, 6, 3, 8, 4, 21, 15, 4334, 166, 761],
    "scenarios with failure": [709, 6, 3, 4, 1, 13, 15, 4334, 1, 100],
    "scenarios without failure": [707, 11, 0, 0, 0, 13, 0, 0, 0, 0],
    "PH": [13, 52, 99, 8640, 17280, 10800, 18, 10, 4, 96],
    # "maximum PH": [42, 169, 324, 8640, 60480, 21600, 19, 20, 6, 192],
    "lead": [2, 2, 2, 288, 720, 720, 2, 2, 2, 2]
})

# Calculate standard deviation and top-1/top-3 labels
std_of_method_performance = df.groupby(['Dataset', 'Technique', 'Flavor'], as_index=False)['Metric'].std(ddof=0).rename(columns={'Metric': 'Std'})
...
if task == 'classification':
    ...
    X = df.drop(columns=[target_variable, secondary_target_variable, 'Dataset', 'Metric', 'Duration', 'Std'])
